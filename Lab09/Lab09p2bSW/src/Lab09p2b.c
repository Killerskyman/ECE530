...
#define UART_DEVICE_ID XPAR_UART_TX_DEVICE_ID
#define UART_BUF_DEVICE_ID XPAR_BUFF_FULL_DEVICE_ID
...
void printUARTmsg(char* msg);

int setupUART(XGpio* uart, int uartID, XGpio* uart_buf, int uartID_buf){
	int status = 0;
	status = XGpio_Initialize(uart, uartID);
	status += XGpio_Initialize(uart_buf, uartID_buf);
	if(status != XST_SUCCESS) return XST_FAILURE;

	XGpio_SetDataDirection(uart, 1, 0x00);
	XGpio_SetDataDirection(uart, 2, 0x00);
	XGpio_SetDataDirection(uart_buf, 1, 0xff);
	return XST_SUCCESS;
}
...
int main(void){
    ...
    status = setupUART(&uart1, UART_DEVICE_ID, &uart1_blk, UART_BUF_DEVICE_ID);
    if(status == XST_FAILURE) printf("Failed to setup secondary uart.\n");
...
//Display temperature in floating point number with 4 digits after decimal point
		...
		sprintf(uartmsg,"TMP101-PL = %8.4f C %8.4f F\r\n", temp_pl_c, temp_pl_f);
...
void printUARTmsg(char* msg){
	...
	for(cnt = 0; cnt<msglen; cnt++){
		while(XGpio_DiscreteRead(&uart1_blk, 1)==1);
		XGpio_DiscreteWrite(&uart1, 1, msg[cnt]);
