...
/*	Modified by Andrew Tebbe	added DisplayImages function			*/
/*				CM 2041													*/
/*				4/29/2016												*/
...
void DisplayDemoPrintMenu(DisplayCtrl *dispPtr)
{
	...
	xil_printf("7 - Display Image 1\n\r");
	xil_printf("8 - Display Image 2\n\r");
	xil_printf("q - Quit\n\r");
	xil_printf("\n\r");
...
int DisplayDemoRun(DisplayCtrl *dispPtr, u32 uartAddr)
{
	...
	while (userInput != 'q')
	{
		...
		case '6':
			...
			break;
		case '7':
			DisplayImages(dispPtr->framePtr[dispPtr->curFrame], dispPtr->vMode.width, dispPtr->vMode.height, dispPtr->stride, 1);
			break;
		case '8':
			DisplayImages(dispPtr->framePtr[dispPtr->curFrame], dispPtr->vMode.width, dispPtr->vMode.height, dispPtr->stride, 2);
			break;
		case 'q':
			break;
		default :
...
void DisplayImages(u32 *frame, u32 width, u32 height, u32 stride, u32 image){
	u32 xcoi, ycoi;
	u32 iPixelAddr;
	u32 wStride;
	u32 wRed, wBlue, wGreen, wColor;
	...
	for(xcoi = 0; xcoi < width; xcoi++)
	{
		...
		for(ycoi = 0; ycoi < height; ycoi++)
		{
			...
			switch(image){
			case 1:
				wRed = (i*j)%((j&i)?(j&i):height);
				wGreen = (i|j)*(i|j);
				wBlue = (i&j)*(i&j);
				break;
			case 2:
				wRed = 255*pow((double)2*abs(i-width/2),(double)abs(j-height/2)/(height/2));
							wGreen = 255*sin(i)*cos(j);
							wBlue = 255*pow((double)abs(j-height/2),(double)abs(i-width/4)/(width/2));
				break;
			}
			wColor = ((u32) wRed << BIT_DISPLAY_RED) | ((u32) wBlue << BIT_DISPLAY_BLUE) | ( (u32) wGreen << BIT_DISPLAY_GREEN);
			frame[iPixelAddr] = wColor;
			/*
			 * This pattern is printed one vertical line at a time, so the address must be incremented
			 * by the stride instead of just 1.
